(function ($) {
  ...
  function dynamicCurrentMenuClass(selector) {
    // Get current page filename (without query or folder)
    let fileName = window.location.pathname.split("/").pop().split("?")[0] || "index.php";
    ...
    selector.find("li a").each(function () {
      ...
      // Check if link matches current page file
      if (linkPage === fileName) {
        $(this).parent("li").addClass("current");

        // Also mark parent dropdowns active
        $(this).parents("li.dropdown").addClass("current");
      }
    });

    // If nothing matches (e.g., homepage is just "/"), mark first menu item as active
